...
| `04-bar.png` | Top bar: workspaces, window title, tray, custom modules. |
...
`10-swaylock.png` (lock screen) is intentionally not shipped — it locks every
output at once, so it's awkward to self-capture. Grab your own with
`./capture.sh` if you want it; the README gallery doesn't reference it.
